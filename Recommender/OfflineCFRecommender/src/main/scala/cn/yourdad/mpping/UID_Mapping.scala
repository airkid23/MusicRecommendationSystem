...
class UID_Mapping {
  ...
  def apply(): UID_Mapping = new UID_Mapping()
  ...
  def insertUserID_Mapping(uid: String, mappingID: Int): Unit ={
    ...
    val sql = "insert into uid_mapping values (?, ?)"
  ...
  def trans(): Unit = {
    ...
    val sql = "select distinct(uid) from rating"
    val stat = conn.createStatement()
    val rs:ResultSet = stat.executeQuery(sql)
    while (rs.next()){
      lst.add(rs.getString("uid"))
    }
    val count = lst.size()
    var a = 1
    for (a <- 1 to count){
