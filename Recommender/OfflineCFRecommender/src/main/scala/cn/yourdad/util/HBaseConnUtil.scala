...
class HBaseConnUtil {
  ...
  def apply(): HBaseConnUtil = new HBaseConnUtil()
  ...
  def scanTable(tableName:String, familyName: String, qulifierName: String) ={
    val config = HBaseConfiguration.create()
    config.addResource(
      new Path("/home/wzj/IdeaProjects/MusicRecommendationSystem/Recommender/DataLoader/src/main/resources/config/core-site.xml")
    )
    ...
    while (r.hasNext){
      ...
      println(Bytes.toString(row.getValue(Bytes.toBytes(familyName), Bytes.toBytes(qulifierName))))
    }
